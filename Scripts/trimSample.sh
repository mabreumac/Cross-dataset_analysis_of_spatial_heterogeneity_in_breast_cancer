...
#Run spaceranger
spaceranger count --id=$SAMPLEID --fastqs=$FASTQ --transcriptome=$REF --sample=$SAMPLEID --image=$JPEG --slide=$SLIDE --area=$AREA --slidefile=$gprfile" >> ${SAMPLE}_trimmed/output_data/sbatchspaceranger_${SAMPLE}_trimmed.sh
